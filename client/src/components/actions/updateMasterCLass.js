...
const  data= {
    ...
    // educationalOrganization :this.state.educationalOrganization,
     ...
     axios.put(`https://hakunamatatasite.herokuapp.com/api/masterClasses/${params.id}`,data).then(response => { 
...
handleDelete= event =>{
  event.preventDefault();
  const { match: { params } } = this.props;
  axios.delete(`https://hakunamatatasite.herokuapp.com/api/masterClasses/${params.id}`).then(response => { 
  console.log(response)
