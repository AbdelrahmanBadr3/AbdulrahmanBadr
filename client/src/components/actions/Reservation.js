...
export class AppliedCourse extends Component {
    ...
    async handleClick() {
        ...
        await axios.put(`https://hakunamatatasite.herokuapp.com/api/coWorkingSpaces/reservation/${this.props.id}/${this.props.roomId}`, data);
    ...
    async handleClickReject() {
        ...
        if(store.get('payload').id==this.props.id){
            ...
            await axios.put(`https://hakunamatatasite.herokuapp.com/api/coWorkingSpaces/reservation/${id}/${this.props.roomId}`, data);
            // window.location.reload(); 
       }else{
           return
       }
